export default class EventsRepository {
	...
	addEvent = async (newEvent) => {
		...
		try {
			const sql =
				"INSERT INTO `custom_events`(`event_date`, `event_name`, `event_description`, `inscription_price`, `group_list`) VALUES (?, ?, ?, ?, ?)";
			const values = [
				newEvent.event_date,
				newEvent.event_name,
				newEvent.event_description,
				newEvent.inscription_price,
				group,
			];
	...
	updateEvent = async (eid, updatedEvent) => {
		...
		try {
			const sql =
				"UPDATE `custom_events` SET event_date = ?, event_name = ?, event_description = ?, inscription_price = ?, group_list = ? WHERE `id_event` = ?";
			const values = [
				updatedEvent.event_date,
				updatedEvent.event_name,
				updatedEvent.event_description,
				updatedEvent.inscription_price,
				group,
				eid,
			];
	...
	changeEventStatus = async (eid, isOpen) => {
		try {
			let sql = "UPDATE `custom_events` SET is_open = ? WHERE `id_event` = ?";

			const values = [!isOpen, eid];
