...
export default class EventsController {
	...
	updateEvent = async (req, res, next) => {
		...
		try {
			...
			const updatedEvent = await this.eventsService.updateEvent(eid, {
				event_date,
				event_name,
				event_description,
				inscription_price,
	...
	changeEventStatus = async (req, res, next) => {
		const { eid } = req.params;
		try {
			const event = await this.eventsService.getEvent(eid);
			...
			await this.eventsService.changeEventStatus(eid, event[0].is_open);
